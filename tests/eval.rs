...
fn create_state<T>(_: &T) -> Context<Ed25519> {
...
fn function_capturing_functions() {
    ...
    const PROGRAM_WITH_REDEFINED_FN: &str = r#"
        ...
        :assert(:bar() == 50);
        fn foo(x) { 3 * x }
        :assert(:bar() == 50); # `bar()` should capture first `foo()` definition.
    "#;
    state.pop_scope();
    state.create_scope();
    let statements = code
...
fn eval_ed25519_chaum_pedersen_proof() {
    ...
    let mut state = create_state(&code);
    assert_eq!(state.evaluate(&statements).unwrap(), Value::Bool(true));
    ...
    assert_matches!(state.evaluate(&statements).unwrap(), Value::Bool(false));
}
